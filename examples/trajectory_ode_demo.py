...
if __name__ == '__main__':
    ...
    if itr % args.test_freq == 0:
        with torch.no_grad():
            # test_y0 = true_y0.unsqueeze(0) - true_yf.unsqueeze(0)
            test_y0 = true_y0.unsqueeze(0)
            
            norm_test_y0 = test_y0 / state_scales
            
            
            pred_y_norm = odeint(func, norm_test_y0, t).to(device).squeeze(1)
            pred_y = pred_y_norm * state_scales
            ...
            # pred_y_squeezed = pred_y.squeeze(1) if pred_y.dim() == 3 else pred_y
            # pred_jerk = func.net(pred_y_squeezed)
            # true_jerk = torch.from_numpy(traj[:, 4]).to(torch.float32).to(device)
            
            loss =  pos_err_weight * torch.mean(torch.abs(pred_y[:, 0] - true_y[:, 0])) + \
                    vel_err_weight * torch.mean(torch.abs(pred_y[:, 1] - true_y[:, 1])) + \
                    acc_err_weight * torch.mean(torch.abs(pred_y[:, 2] - true_y[:, 2]))

            # loss_tracking.append(loss.item())
                    
            print('Iter {:04d} | Total Loss {:.6f}'.format(itr, loss.item()))
            visualize_ruckig(traj, pred_y, t, ii, func, False)
    ...
    if itr % args.curriculum_freq == 0:
        batch_length = min(batch_length + args.batch_time, len(t))
        print(f"Curriculum update: Increasing batch_length to {batch_length}")
    """
